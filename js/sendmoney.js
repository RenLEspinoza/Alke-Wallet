$(document).ready(function () {
  ...
  $("#btn-confirmar-envio").click(function () {
    ...
    let contactoEncontrado = listaContactos.find(
      (c) => c.nombre.toLowerCase() === nombreBuscar.toLowerCase(),
    );

    if (!contactoEncontrado) {
      mostrarAlerta(
        "El contacto ingresado no existe en tu lista.",
        "alert-danger",
      );
      return;
    }
    ...
    mostrarAlerta(
      `¡Transferencia de $${monto.toLocaleString("es-CL")} a ${contactoEncontrado.nombre} realizada con éxito!`,
      "alert-success",
    );
    ...
    // 2. Si la caja estaba vacía, le ponemos un texto por defecto. (no deberia estar vacia, just in case).
    ...
    historial.push({
      ...
      detalle: "A: " + contactoSeleccionado,
  ...
  $("#btn-guardar-contacto").click(function () {
    ...
    let nuevoRut = $("#rutContacto").val()
      ? $("#rutContacto").val().trim()
      : "N/A";
    let nuevoBanco = $("#bancoContacto").val()
      ? $("#bancoContacto").val().trim()
      : "Banco";
    ...
    mostrarAlerta(
      `¡Contacto "${nuevoNombre}" agregado exitosamente!`,
      "alert-success",
    );
  });
